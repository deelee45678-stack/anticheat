...
/* SHA-256, FIPS 180-4, self-contained (no external dependencies). */
...
void sha256_final(sha256_ctx *ctx, unsigned char hash[SHA256_DIGEST_LEN]) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        hash[4 + i]   = (unsigned char)((ctx->state[1] >> (24 - i * 8)) & 0xff);
        hash[8 + i]   = (unsigned char)((ctx->state[2] >> (24 - i * 8)) & 0xff);
        hash[12 + i]  = (unsigned char)((ctx->state[3] >> (24 - i * 8)) & 0xff);
        hash[16 + i]  = (unsigned char)((ctx->state[4] >> (24 - i * 8)) & 0xff);
        hash[20 + i]  = (unsigned char)((ctx->state[5] >> (24 - i * 8)) & 0xff);
        hash[24 + i]  = (unsigned char)((ctx->state[6] >> (24 - i * 8)) & 0xff);
        hash[28 + i]  = (unsigned char)((ctx->state[7] >> (24 - i * 8)) & 0xff);
    }
}
